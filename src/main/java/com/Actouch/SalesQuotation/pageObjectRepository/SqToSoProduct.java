	...
	public class SqToSoProduct {
	   ...
	   @FindBy(xpath="//a[contains(.,'Convert To Sales Order')]")
	   WebElement convertBtn;
	   @FindBy(id="btnSavePo")
	   WebElement saveBtn;
	   @FindBy(xpath="//button[contains(.,'Print Sales Order')]")
	   WebElement confMsgBtn;
	   
	   
	   public void sqtoSO (String Customer_Id,String Product_Id,String Quantity,String unit_price,String taxCode) throws InterruptedException, EncryptedDocumentException, InvalidFormatException, IOException
	   {
		 ...
		 for(int i=1;i<10;i++)
		  ...
		  confMsgBtn.click();
		   String act2 = actval.getText();
		    Assert.assertEquals(act2, exp,"Non Inventory product not saved after executing case == fail");
	  		System.out.println("Non Inventory product saved after executing case == pass");
