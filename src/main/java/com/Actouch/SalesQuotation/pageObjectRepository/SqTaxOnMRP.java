...
public class SqTaxOnMRP {
   ...
   public void taxOnMRP(String Customer_Id,String Product_Id,String Quantity,String mrp,String taxCode) throws InterruptedException, EncryptedDocumentException, InvalidFormatException, IOException
   {
	 ...
	 prdSq_1Edt.sendKeys(Product_Id);
	   Thread.sleep(1000);
     	...
     	Thread.sleep(1000);
     	boolean expB= chkTaxIncl.isSelected();
   	 
   	 Assert.assertEquals(b1, expB);
   		System.out.println("When Tax on MRP is enable for product check box for Incl.Tax is always selected == pass");
	 ...
	 for(int i=1;i<10;i++)
	  ...
	  searchId.sendKeys(exp);
	   String act2 = actval.getText();
	    Assert.assertEquals(act2, exp,"SQ with taxOnMRP not saved after executing case == fail");
		System.out.println("SQ with taxOnMRP saved after executing case == pass");
