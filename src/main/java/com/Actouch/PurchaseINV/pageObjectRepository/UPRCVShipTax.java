...
public class UPRCVShipTax {
	...
	WebElement ShipTaxEdt;
	@FindBy(id="shipCharges")
	WebElement ShipChargeEDT;
	@FindBy(id="PORShipTax-select")
	WebElement ShippingTaxEdt;
	@FindBy(id="btnSave")
	WebElement SaveEDT;
	...
	WebElement ViewRCV;
	@FindBy(xpath="//label[@data-bind ='html: PORInventory.shipTax']")
	WebElement Result;
	public void ReFerence(String ShipCharge,String ShippingTax) throws InterruptedException
	{
		...
		ActionsEdt.click();
		UpdateRCV.click();
		ShipChargeEDT.sendKeys(Keys.chord(Keys.CONTROL,"a"),ShipCharge);
		ShipTaxEdt.sendKeys(Keys.chord(Keys.CONTROL,"a"),"YES");
		act.sendKeys(Keys.ARROW_DOWN).perform();
		act.sendKeys(Keys.ENTER).perform();
		ShippingTaxEdt.sendKeys(Keys.chord(Keys.CONTROL,"a"),ShippingTax);
		act.sendKeys(Keys.ARROW_DOWN).perform();
		act.sendKeys(Keys.ENTER).perform();
		String Expected=ShippingTaxEdt.getAttribute("value");
		SaveEDT.click();
		OKEdt.click();
		Thread.sleep(3000);
		SearchPOR.sendKeys(Exp);
