...
import com.Actouch.SalesINVSiteLoc.pageObjectRepository.SHPMultiproduct;
import com.Actouch.SalesINVSiteLoc.pageObjectRepository.SOMultiproduct;
public class ShipmentSiteLoc {
	WebDriver driver;
	...
	public void Shp_Multiproduct() throws Exception
	{
	    ...
	    String total =SO_Excellib.getExcelDat("Shipment", 18, 15);
		  ...
		  SoftAssert Assert = new SoftAssert();
			...
			SHPMultiproduct.payltrDB("jdbc:mysql://178.162.192.40:3306/", "actouchuser", "actouchuser","select * from actouch.shp_mstr where  ENTITY_ID = 399807654 and Invoice_ID ='"+SHPMultiproduct.Excepted+"'", "CUST_ID","CUST_NAME","TOTAL_AMT","TOTAL_TAX");
			Assert.assertEquals(custID,SHPMultiproduct.CUST_ID,"CustID is not correct");
			 ...
			 Assert.assertEquals(Total_TXT, totalTaxExp,"Total Tax coming as wrong ");
				...
				SHPMultiproduct.payltrDB2("jdbc:mysql://178.162.192.40:3306/", "actouchuser", "actouchuser", "select * from actouch.tax_hist where ENTITY_ID = 399807654 and DOC_ID='"+SHPMultiproduct.Excepted+"' and SITE_ID = 4 order by  sl_no asc", "TAX_ID", "TAXABLE_AMT", "GROSS_AMT", "SGST_TAX_AMT", "CGST_TAX_AMT");
				 Assert.assertAll();
	}
